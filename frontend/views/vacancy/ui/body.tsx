...
function GlanceRow({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="flex items-center justify-between gap-4 border-b border-white/[0.06] pb-3 last:border-0 last:pb-0">
      <dt className="font-mono-tech text-[10px] uppercase tracking-[0.16em] text-main-mist/45">{label}</dt>
      <dd className={accent ? "text-sm font-medium text-main-accent-t1" : "text-sm text-main-mist/85"}>{value}</dd>
    </div>
  );
}
        ...
        <div className="grid gap-12 lg:grid-cols-[1fr_360px]">
          <div className="space-y-10">
            <Block title="About the role">
              <p className="text-sm leading-relaxed text-main-mist/65 sm:text-base">{vacancy.about}</p>
            </Block>
          ...
          <aside className="h-fit lg:sticky lg:top-28">
            <div className="rounded-2xl border border-white/[0.07] bg-main-card/40 p-6">
              <p className="mb-4 font-mono-tech text-[10px] uppercase tracking-[0.2em] text-main-mist/45">At a glance</p>
              <dl className="space-y-3">
                ...
                <GlanceRow label="Compensation" value={vacancy.compensation} accent />
              </dl>
              <a
                href="#apply"
