...
function Stat({ value, label, className }: { value: string; label: string; className: string }) {
  return (
    <div>
      ...
      <p className="mt-1 font-mono-tech text-[10px] uppercase tracking-[0.2em] text-main-mist/45">{label}</p>
    </div>
  );
}

export function HiringHero({ roleCount, teamCount }: { roleCount: number; teamCount: number }) {
  return (
    <section className="relative overflow-hidden bg-main-black pt-40 pb-16">
      <div className="container">
        <p className="mb-5 font-mono-tech text-[11px] uppercase tracking-[0.34em] text-main-accent-t1">Careers · Join the fund</p>
        <h1 className="max-w-3xl font-serif-display text-4xl font-light leading-tight text-white sm:text-6xl">
          Build coastal Vietnam&apos;s <span className="font-serif italic text-main-accent-t1">next decade</span> with us.
        </h1>
        ...
        <div className="mt-14 flex items-center gap-10 border-t border-white/[0.06] pt-7">
          <Stat value={String(roleCount)} label="Open roles" className="text-main-accent-t3" />
          <span className="h-8 w-px bg-white/10" />
          <Stat value={String(teamCount)} label="Teams hiring" className="text-main-accent-t1" />
          <span className="h-8 w-px bg-white/10" />
